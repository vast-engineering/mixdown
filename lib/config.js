...
Config.prototype.init = function(callback) {
	...
	this.getExternalConfig(function(err, extConfig) {
      ...
      if (services && services.length) {
        that.overlays = _.filter(services, function(s) { return s.overlay === true; });
        that.services = _.reject(services, function(s) { return s.overlay === true; });
      }
      else {
        that.services = [that.config.app];
      }
...
Config.prototype.initServices = function(callback) {
	...
	var that = this;
  var appConfig = _.cloneDeep(defaultApp);

	// enumerate and create the apps
	_.each(this.overlays, function(overlay) {
    _.bind(mergeSection, appConfig, overlay, 'plugins', overlay.override)();
	});

  // enumerate and create the apps
  _.each(this.services, function(service) {

    _.bind(mergeSection, service, appConfig, 'plugins')();
