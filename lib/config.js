...
Config.prototype.initLogger = function() {
	...
	global.logger = mdLogger.create(this.config.logger);
...
Config.prototype.getExternalConfig = function(callback) {
	...
	pluginUtil.use({
		plugin: this.services || { module: "mixdown-config-filesystem", options: { paths: [ './mixdown-config-services' ] }},
		app: app
	});
...
Config.prototype.initServices = function(callback) {
	...
	if (!this.services) {
		_.isFunction(callback) ? callback(new Error('There are no services to initialize.')) : null;
		return;
	}
	...
	_.each(this.services, function(service) {
		...
		_.bind(mergeSection, service, raw.app, 'plugins')();
		service.main = raw.main;
